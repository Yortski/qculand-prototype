...
export default function LibraryPlayer() {
  ...
  // Camera control - start with better angle
  const cameraAngle = useRef(Math.PI); // Face forward (180 degrees)
  const cameraPitch = useRef(0.4); // Slight downward angle
  const cameraDistance = useRef(3.0); // Zoomed out more
  ...
  // Library room boundaries (5m x 5m room)
  const bounds = {
    minX: -2.2,
    maxX: 2.2,
    minZ: -2.2,
